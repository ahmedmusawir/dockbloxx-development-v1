...
- UI control: bordered `<button>` elements styled exactly like the Pole Size buttons (single-select; selected = blue). ~~Radio input `name` attribute `poleMaterial`~~ superseded by Director override 2026-09-08. Nothing selected on load.
...
| 2026-09-08 | Director (override) | Placement: BELOW the entire Pole Size block, above Current Price (not under Pole Shape Styles). Control: two bordered buttons `Metal` / `Wood` styled like Pole Size, none selected on load, selected = blue. All other rows stand. | UI control line; DoD §2.1; RECON headline 6; 01_SOLUTION Step 3; AC1 |
| 2026-09-08 | Engineer (Step 6 evidence) | Cart does not merge across materials (2 line items, same `variation_id`); merge identity unchanged. Step 5: all renderers GENERIC, `orderTransform.ts` untouched. | consumer assumptions (confirmed) |
