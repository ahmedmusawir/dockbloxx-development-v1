...
| AC5 | TEST BUY (Square / 2" / Wood on `whos-your-caddie`, product 3157) → staging admin order line shows `Pole Material: Wood` in the nested block with Pole Style | screenshot + order id | staging WP admin | Director | Pending |
| AC6 | WC REST readback of that order: `line_items[].meta_data[key="variations"].value[]` contains `{name:"Pole Material", value:"Wood"}` — identical string | CLI readback | staging REST | agent | Pending |
| AC7 | TEST BUY with no selection → admin shows `Pole Material: Unknown` as a visible row (renderer does not filter `Unknown`, per #14889 `Version: Unknown`); REST matches | screenshot + CLI | staging | Director + agent | Pending |
...
- Production build (part of AC10): not run by the Engineer yet — dev server holds `.next`. Run before Gate Q.
