...
- `next build` → **NOT RUN** (dev server holds `.next`; see limitations)
...
### Known limitations
- **TEST BUY not performed** (Director's). CONTRACT trace is proven up to the cart item; payload → WC REST → admin display (AC5/AC6/AC7) awaits the buy. Engineer performs the REST readback on the order id.
- **Production build not run** (see above).
- WP-side renderer unproven (GAP-2). Expected: `Pole Material: Wood` / `Unknown` rows appear.
...
### Handoff
→ QA Lead per QA_PLAYBOOK: intake → contract extraction → Gate Q (dev repo vs staging) → Gate D after promotion (Track B on production first). Director drives manual steps one test at a time. **Engineer STOPS here.**
