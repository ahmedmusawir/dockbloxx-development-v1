...
| Typecheck + production build | compiles clean | **tsc PASS** (`npx tsc --noEmit`, tests included by tsconfig). **Production build NOT RUN** — `next build` would write to `.next` while the Director's dev server holds it (G-NPM-2 class risk). Needs Director to stop the dev server or approve the run. | — |
...
| CONTRACT trace | `Wood` survives ACF → JSON → button → cart → payload → REST readback | **PARTIAL** — ACF `wood="Wood"` (cache-busted GET) → category JSON `poleMaterials.wood="Wood"` (curl) → button label `Wood` → cart `{name:"Pole Material",value:"Wood"}` (localStorage) ✔. Payload → REST readback **requires the Director's TEST BUY**; `orderTransform.ts` unchanged and sends `item.variations` unfiltered (L110–114) | step3/step4 verification; step5 evidence |
...
### Key values captured — ACF `metal`: `Metal` · ACF `wood`: `Wood` · TEST BUY order id: **pending (Director)** · REST readback entry: **pending**
### Known limitations / open risks (carry into Claim Package)
- Production build not yet run (dev server holds `.next`).
- WP-side renderer (GAP-2) unproven until TEST BUY; expected to show `Pole Material: Wood` and `Pole Material: Unknown` rows (renderer does not filter `Unknown`, per Director's #14889 screenshot).
