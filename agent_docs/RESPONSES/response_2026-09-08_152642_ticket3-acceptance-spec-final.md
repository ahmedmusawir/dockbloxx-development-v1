...
| AC5 | TEST BUY (`whos-your-caddie`, product 3157; executed as Square / 4" / **Metal** — Director's choice, literal differs from the Wood example, pipe identical) → staging admin order line shows `Pole Material: Metal` in the nested block with Pole Style | screenshot + order id | staging WP admin | Director | **PASS (Director, order #14893, screenshot)** |
...
| AC7 | TEST BUY with no selection → admin shows `Pole Material: Unknown` as a visible row (renderer does not filter `Unknown`, per #14889 `Version: Unknown`); REST matches | screenshot + CLI | staging | Director + agent | **WAIVED by Director 2026-09-08** — basis: Step 4 browser run (cart seeds `Unknown`) + #14889 (renderer prints `Unknown` rows). No buy performed. |
