...
### Known limitations
- **TEST BUY performed by the Director:** staging order **#14893**, `whos-your-caddie`, Square / 4" / **Metal** (not Wood — same pipe, different literal). Admin block shows `Pole Material: Metal` (Director screenshot, AC5). REST readback shows exactly one `{"name":"Pole Material","value":"Metal"}` (AC6, Engineer). Order id inferred from the staging listing — Director to confirm.
- **AC7 Director-waived** (no-selection buy not performed). Basis: Step 4 browser run + #14889 renderer behaviour; additionally order #14894 (today, no selection) carries `Pole Material: Unknown` over REST.
...
### Handoff
**HANDED OFF to QA Lead 2026-09-08** per QA_PLAYBOOK: intake → contract extraction → Gate Q (dev repo vs staging) → Gate D after promotion (Track B on production first). Director drives manual steps one test at a time. Engineer pre-checks on AC1–AC4, AC6, AC8–AC10 are claims for QA to re-verify; AC5 Director-evidenced; AC7 waived. **Engineer STOPS here** and resumes only on QA findings routed back (in-scope Critical/High → checkpoint-gated fix).
