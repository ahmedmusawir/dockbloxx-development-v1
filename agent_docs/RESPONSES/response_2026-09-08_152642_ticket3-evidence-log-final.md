...
### Key values captured — ACF `metal`: `Metal` · ACF `wood`: `Wood` · TEST BUY order id: **#14893** (2026-09-08T02:06:01, Square / 4" / Metal, variation_id 3184; id inferred from REST listing — Director to confirm) · REST readback entry: `{"name":"Pole Material","value":"Metal"}` — exactly one, in `meta_data[key="variations"].value`
...
- Product `whos-your-caddie` (3157), **Square / 4" / Metal** (Director chose Metal; AC5 evaluated on `Metal` instead of `Wood` — same pipe, different literal). Admin order block shows `Pole Material: Metal` (Director screenshot). Order id: **#14893** (inferred from today's staging order listing; Director to confirm). REST readback (AC6): **PASS** — see RESPONSES ac6-rest-readback. Bonus evidence: today's order #14894 (Fillet Bloxx, Octagon / 4", no selection) carries `{"name":"Pole Material","value":"Unknown"}` over REST — the Unknown pipe end-to-end at the REST layer, supporting the AC7 waiver.
- **AC7 Director-waived:** no-selection buy not performed. Basis: Step 4 browser run proves cart-level `Pole Material: Unknown` seeding; #14889 proves the WP renderer prints `Unknown` rows (`Version: Unknown`). Waiver recorded in ACCEPTANCE_SPEC.
